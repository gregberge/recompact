/* eslint-disable no-cond-assign, no-shadow, func-names, import/no-unresolved, import/extensions */
...
function build() {
  ...
  const template = fs.readFileSync(path.resolve(__dirname, 'website-template.ejs'), 'utf-8')
  const html = ejs.render(template, {
    body: $.html().trim(),
    version,
  })

  fs.writeFileSync(path.join(docPath, `${version}.html`), html)
